...
export function buildSettlementWordHtml(loan: LoanDocumentRecord, options?: DocumentRenderOptions) {
  ...
  const body = `
    ...
    <div class="official-inline" style="grid-template-columns: 1.35fr 1fr 1fr; direction: rtl; text-align: right; align-items: center;">
      ...
      <span>التوقيع: <span class="signature-line" style="position:relative;display:inline-block;overflow:visible;">${applicantSignature ? `<img style="position:absolute;bottom:0;left:0;width:100%;height:40px;object-fit:contain;mix-blend-mode:multiply;" src="${applicantSignature.dataUrl}" alt="توقيع الموظف" />` : ''}</span></span>
